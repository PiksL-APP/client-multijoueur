...
| **Garage de peinture** | bleu | 60 px | On y entre **en voiture** : les étoiles tombent à zéro, la tôle est réparée et la voiture ressort **repeinte** (dix couleurs, jamais celles d'un gang) — la peinture la suit chez tout le monde, garée ou volée. C'est le *respray* du guide. Changer de voiture, sans garage, **sème d'un cran** (pas une voiture de police). | §5.2 |
| **Atelier** | bleu, **cinq pastilles de couleur** | 60 px | Un garage sur deux vend aussi des modifications. On se gare **sur la pastille qu'on veut** et `F` achète : plaques (450 $), mitrailleuse de bord (950 $), mines (750 $), huile (550 $), bombe (650 $ — armée en descendant ; `H` à pied la fait sauter, ou le premier voleur). Détail : `claude/atelier-de-modification.md`. | §7.2, phase 7 — **fait** |
...
| **Sa planque** | maison violette | 70 px | À pied, `F` **entre chez soi**. Au volant, `F` dépose l'argent sans descendre (les travaux, eux, se paient au coffre, à l'intérieur). Y arriver en voiture la **range au garage** si on l'a acheté. | proche du « Jesus Saves » §4.4, mais gratuit |
| **Chez soi — le coffre** | disque **jaune** | 0,9 tuile | `F` ouvre le **menu de la planque** : déposer, retirer, et les trois travaux avec leur prix et ce qu'ils font — coffre-fort (1 800 $ : sans lui, le matelas ne cache que 4 000 $), arsenal (3 200 $ : on garde son arme en mourant), garage (5 000 $ : la voiture ramenée vous attend après la mort), dans cet ordre. `E` **retire** tout sans ouvrir le menu. | — |
...
| **Camion de pompiers** | caisse rouge, échelle sur le toit | — | Il **arrive tout seul** sur les incendies et les éteint (`VilleVivante`, lance à eau) — et on voit le jet. Volé, **ESPACE arrose** au lieu de tirer : le jet (240 px, cône étroit) éteint ce qui brûle devant et **couche les passants** sans les blesser. Avec le **lance-flammes** (laissé par le patron d'un repaire à la première mission rendue), `F` bascule la lance eau/feu : le feu grille les passants, brûle les voitures et allume des foyers. | §6.2, phase 3 — **fait**, canon à eau et lance-flammes compris |
...
l'armurerie s'ouvre à vous, et le gang n'y renaît plus. Et **un repaire se
défend** : abattre un des hommes du tag, ou ouvrir un raid, met tous ses
camarades à vos trousses pendant 25 s, quoi que leur gang pense de vous par
ailleurs — c'est le repaire qui se fâche, pas la jauge de respect. Détail :
`claude/repaires-de-gang.md`.

## Les missions du repaire

Au palier allié, **le patron** (au bout de la table, marque orange) confie ce
qu'une cabine ne confie pas : **la mallette** d'un rival, posée chez lui au
milieu de ses gars, à ramasser comme un colis puis à **rapporter sur le tag**
du patron (120 s, 2 200 $) ; ou **la tête de son lieutenant**, un homme trois
fois plus dur coiffé d'une flèche orange (100 s, 2 600 $). Un seul travail en
main à la fois — mission ou contrat — et la ligne du HUD dit MISSION. Le radar
pointe la mallette, puis le tag ; le repaire du lieutenant. Prendre la
mallette, ou toucher le lieutenant, sort le repaire au complet.
...
| ~~Phase 7 — garages de modification~~ | **Fait.** Pas un second garage : un garage sur deux vend en plus, sur cinq pastilles où l'on se gare. La couleur de la carrosserie change vraiment depuis (`CONCEPTION-ATELIER.md`). Et depuis le 11/09 la bombe est **déclenchée** : `H` à pied fait sauter ses voitures piégées, le voleur qui prend le volant y reste. |
...
- **l'église « Jesus Saves »** (§4.4) : sauvegarde payante. Notre planque en
  tient lieu, mais elle est gratuite et ne coûte pas de score. C'est le
  dernier point du guide qui reste.

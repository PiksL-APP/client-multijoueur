...
func _input(evenement: InputEvent) -> void:
	...
	# Le second bouton est AU-DESSUS du premier, pas à côté : le pouce droit
	# monte plus facilement qu'il ne s'écarte vers le bord de l'écran.
	var action_centre := bouton_centre - Vector2(0.0, 168.0)

	if evenement is InputEventScreenTouch:
		var t := evenement as InputEventScreenTouch
		if t.pressed:
			if t.position.distance_to(action_centre) <= 74.0:
				_doigt_action = t.index
				bouton_action_tenu = true
			elif t.position.distance_to(bouton_centre) <= 88.0:

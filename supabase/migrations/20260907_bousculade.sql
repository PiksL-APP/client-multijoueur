-- Troisième jeu : la Bousculade. La liste des jeux connus est fermée (contrainte
-- et fonction de dépôt) : on l'ouvre au nouveau venu, et à lui seul.
alter table public.jeu_parties drop constraint if exists jeu_parties_jeu_check;
alter table public.jeu_parties
  add constraint jeu_parties_jeu_check check (jeu in ('carnage', 'enigme', 'bousculade'));
...
declare
  ...
  v_recentes integer;
begin
  if p_jeu is null or p_jeu not in ('carnage', 'enigme', 'bousculade') then
